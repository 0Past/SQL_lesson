...
from os import path
import math
from sem1 import settings, bonus, explosion, functions, rocket, asteroid
...
''' Импорт изображений '''
asteroid_images = {}
asteroid_images['medium'] = scale(pygame.image.load(path.join(img_dir, "asteroid.png")).convert_alpha(), (
settings.ASTEROID_SIZE_MEDIUM, settings.ASTEROID_SIZE_MEDIUM))
asteroid_images['big'] = scale(pygame.image.load(path.join(img_dir, "big.png")).convert_alpha(), (
settings.ASTEROID_SIZE_BIG, settings.ASTEROID_SIZE_BIG))
    ...
    for hit in hits:
        if not player.hidden and not player.invisible:
            ...
            if hit.type == 'bomb':
                ...
                expl_sound.play()
                score += int(settings.ASTEROIDS_COUNT / 2)
                for i in range(int(settings.ASTEROIDS_COUNT / 2)):
                    a = asteroids_list[i]
